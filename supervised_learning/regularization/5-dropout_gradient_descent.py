...
def dropout_gradient_descent(Y, weights, cache, alpha, keep_prob, L):
    ...
    Parameters:
    Y (ndarray):
    One-hot numpy array of shape (classes, m) that contains the correc
    labels for the data.
    weights (dict):
    A dictionary containing the weights and biases of the neural network.
    cache (dict):
    A dictionary containing the outputs of each layer during forward
    propagation.
    alpha (float):
    The learning rate.
    keep_prob (float):
    ...
    applying gradient descent on each layer.
    """
